...
% last modified by Lujia Feng Thu Jun 16 23:18:43 SGT 2016                       %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if isequal(rake0,rakeX)                 % rake0==rakeX means rake is fixed
    ...
    Aineq = zeros(slip_num,slip_num); bineq = zeros(slip_num,1);
...
%  flt = [ x1 y1 x2 y2 z1 z2 dip rake rs ts rake0 rakeX rs0 rsX ts0 tsX ]        %
    ...
    for ii = 1:flt_num
        ...
        if rakevary0 < 0
            if rakevary0 <= -90  
                fprintf(1,'GTdef_fault4uni WARNING: rake0-rake<=-90 is not allowed, reset rake0-rake to be -89.99\n');
                rakevary0 = -89.99;
            end
            ...
            fprintf(1,'GTdef_fault4uni WARNING: rake0>=rake is not allowed, reset rake0 to be rake\n');
            Aineq(ii,ii)         =  0; 
            Aineq(ii,ii+flt_num) = -1;
        end
        ...
        if rakevaryX > 0
            if rakevaryX >= 90  
                fprintf(1,'GTdef_fault4uni WARNING: rakeX-rake>=90 is not allowed, reset rakeX-rake to be 89.99\n');
                rakevaryX = 89.99;
            end
            ...
            fprintf(1,'GTdef_fault4uni WARNING: rakeX<=rake is not allowed, reset rakeX to be rake\n');
    ...
    if strcmpi(etype,'homogeneous')
        ...
        [ Mrs90 ] = GTdef_fault2p_to_okada(x1,y1,x2,y2,z1,z2,dip,ss,ds,ts);
        ...
        [ Mrs90 ] = GTdef_fault2p_to_edcmp(x1,y1,x2,y2,z1,z2,dip,ss,ds,ts);
