...
% last modified Lujia Feng Fri Oct 17 17:39:46 SGT 2014                         %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
tic
    ...
    if strcmp(coord,'geo_polyconic')
       [nod_xx,nod_yy] = latlon_to_xy(nod.lon,nod.lat,lon0,lat0,0);
    end
    if strcmp(coord,'local')
...
tic
	...
	if strcmpi(grnflag,'on')
            GTdef_save_greensfns(cflt_name,pnt,flt1.flt(ii,:),Xgrn1);
	end
    end
toc
end
...
tic
	...
	% save green's functions
	if strcmpi(grnflag,'on')
            GTdef_save_greensfns(cflt_name,pnt,flt2.flt(ii,:),Xgrn2);
	end
    end
toc
end
...
tic
	...
	% save green's functions
	if strcmpi(grnflag,'on')
            GTdef_save_greensfns(cflt_name,pnt,flt3.flt(ii,:),Xgrn3);
	end
    end
toc
end
...
tic
	...
	% save green's functions
	if strcmpi(grnflag,'on')
            GTdef_save_greensfns(cflt_name,pnt,flt4.flt(ii,:),Xgrn4);
	end
    end
toc
end
...
tic
    for ii = 1:flt5.num
       ...
       cgrn_name = flt5.grname{ii};
       % read greens functions
       [ siteList,siteloc,vertices,grnList,grnfns ] = PyLith_read_greensfns(cgrn_name);
       ...
       GTdef_fault5(flt5.flt(ii,:),subflt.flt(sub_ind,:),vertices,grnfns,smooth,surf);
...
tic
    ...
    flt1.out = flt1.flt; flt2.out = flt2.flt; flt3.out = flt3.flt; flt4.out = flt4.flt; flt5.out = flt5.flt;
    ...
    GTdef_output(foutName,coord,'none','none',0,rigidity,poisson,earth,edgrn,layer,...
    		 flt1,flt2,flt3,flt4,flt5,subflt,dip,pnt,bsl,prf,grd,nod,mod_info);
toc
else
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% inversion %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fprintf(1,'\n............. doing inversion .............\t');
    ...
    for ii = 1:betaNum
        ...
        tic
            ...
            % faults info
            [ flt1.out,flt2.out,flt3.out,flt4.out,flt5.out,subflt.out,subflt.outname ] = ...
	    ...
	    GTdef_forward(Xgrn,Bgrn,Ngrn,sm_use,sm_abs,lb,ub,xx,pnt,bsl,nod,smooth);
            ...
            GTdef_output(foutName,coord,smooth,surf,bt,rigidity,poisson,earth,edgrn,layer,...
                         flt1,flt2,flt3,flt4,flt5,subflt,dip,pnt,bsl,prf,grd,nod,mod_info(ii,:));
        toc
    end
