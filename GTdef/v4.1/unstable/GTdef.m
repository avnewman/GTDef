...
if strcmp(modspace.coord,'geo') || strcmp(modspace.coord,'geo_polyconic')
    if isempty(modspace.origin)
        ...
        % for other fault types, if origin is missing (not needed when not using
...
tic
    for ii = 1:flt5.num
        ...
        % seems to me that modspace.lb and modspace.up needs to be updated
        % with information within newflt for all subpatches that are not hard-wired
        % as zero  -- AVN 8/16/2023   //TODO
